...
exports.updateProduct = async (req, res) => {
  try {
    ...
    if (req.file) {
      if (data.image) {
        // jika data sebelumnya mempunyai gambar, maka gambar akan di hapus dan di ganti dengna gambar yg baru di upload
        const imagePath = path.join(global.path, 'uploads', 'products', data.image) // mengambil path gambar
...
exports.deleteProduct = async (req, res) => {
  try {
    ...
    if (product.image) {
      const imagePath = path.join(global.path, 'uploads', 'products', product.image)
      // mengambil jalur path image
      console.log(imagePath)
      fs.access(imagePath, fs.constants.R_OK).then(() => {
        fs.rm(imagePath)
        // menghapus file berdasarkan path
